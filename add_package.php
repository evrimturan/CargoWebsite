...
if($mult_choice == on){
  $sql_username = "SELECT userName FROM send WHERE send.packageID = (SELECT max(package.packageID) AS Pid_max FROM package)";
  ...
  $sql_store_old = "SELECT sid FROM destination WHERE packageID = (SELECT max(packageID) as Pid_max FROM package)";
  $res_user = $con->query($sql_username);
  $test_row = mysqli_fetch_assoc($res_user);
  ...
  $res04 = $con->query($sql_store_old);
  $test_row4= mysqli_fetch_assoc($res04);

  $user_name = $test_row["userName"];
  $addr = $test_row4["sid"];
  ...
  $package_type = "";
  if($weight <= 0){
    header("Location:package.php?firstname=".$_GET["firstname"]."&result=false"."&fail=weight_error");
    die("Error!");
  }
  if($weight <= 1){
    $cost = $cost + 1;
  ...
  if($timeliness == "Overnight"){
    ...
    header("Location:package.php?firstname=".$_GET["firstname"]."&result=false"."&fail=NULL_return_time"."&".$timeliness);
    die("Error!");
  }
  if($content == "normal"){
    $cost = $cost + 0;
    ...
    header("Location:package.php?firstname=".$_GET["firstname"]."&result=false"."&fail=NULL_return_type");
    die("Error!");
  }
  ...
  if(mysqli_query($con,$sql_package)){
      ...
      $sql_user_insert = "INSERT INTO send VALUES('$temp_pack','$user_name')";
      $sql_get_storeid = "SELECT sid FROM works WHERE userName='".$_GET["firstname"]."'";
      $res_src_id = $con->query($sql_get_storeid);
      $row_temp_2 = mysqli_fetch_assoc($res_src_id);
      if(!empty($row_temp_2["sid"])){
        $sql_source = "INSERT INTO source (`sid`,`packageID`) VALUES ('".$row_temp_2["sid"]."','$temp_pack')";
          ...
          if(mysqli_query($con,$sql_user_insert)){
            if(mysqli_query($con,$sql_source)){
              if(mysqli_query($con,$sql_destination)){
                header("Location: package.php?tracknum=$temp_shipment"."&result=true"."&firstname=".$_GET["firstname"]);
              }else{
                header("Location: package.php?result=false"."&firstname=".$_GET["firstname"]);
  ...
  if($timeliness == "overnight"){
    ...
    header("Location:package.php?firstname=".$_GET["firstname"]."&result=false"."&fail=NULL_return_time");
    die("Error!");
  }
  if($content == "normal"){
    $cost = $cost + 0;
