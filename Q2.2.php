...
<?php
/*
Find the cities that ships the min number of the
packages between April 20, 2013 and May 15, 2013.
*/

require "connectdb.php";
$recent_sql = "SELECT count(*) AS cnt, city FROM store, (SELECT sid FROM source,
...
GROUP BY city
HAVING count(*) = (SELECT min(cnt) from (SELECT count(*) AS cnt,city FROM store,
(SELECT sid FROM source, (SELECT packageID,include.trackingNum FROM include,
(SELECT trackingNum FROM shipment
WHERE dateOfShipment BETWEEN '2013/04/20' AND '2013/05/15')AS myTable
WHERE myTable.trackingNum = include.trackingNum)AS myTable2
WHERE myTable2.packageID = source.packageID)AS myTable3
WHERE myTable3.sid = store.sid
...
if(mysqli_query($con,$recent_sql)){
  ...
  if($res_table->num_rows > 0)
  {
    ...
    {
      echo "<br>";
      echo "City: " . $row["city"]. "<br>". " -Min Amount: " . $row["cnt"]. "<br>";
    }
  }
}
else{
